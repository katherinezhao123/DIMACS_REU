#!/bin/bash
#SBATCH --job-name=lora-liquidai-test12
...
#SBATCH --output=test12/lora_out.txt   # stdout — %j is the job ID
#SBATCH --error=test12/lora_err.txt    # stderr separately so errors are easy to find
...
mkdir -p ./test12/model ./test12/checkpoints
...
START=$(date +%s)
python3 /cache/home/kz498/Research/Train/lora/liquidai/lora_val/lora_liquidai.py \
        --r 8 \
        --lora_alpha 8 \
        --lr 5e-5 \
        --num_train_epochs 4 \
        ...
        --warmup_ratio 0.06 \
        --lr_scheduler linear \
        --output_dir ./test12/checkpoints \
        --save_dir ./test12/model 
