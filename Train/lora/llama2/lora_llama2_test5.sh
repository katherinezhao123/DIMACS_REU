#!/bin/bash
#SBATCH --job-name=lora-llama2-test5
...
#SBATCH --output=test5/lora_out.txt   # stdout — %j is the job ID
#SBATCH --error=test5/lora_err.txt    # stderr separately so errors are easy to find
...
mkdir -p ./test5/model ./test5/checkpoints

# Monitor GPU memory
nvidia-smi --query-gpu=timestamp,memory.used,memory.free,utilization.gpu \
    --format=csv -l 5 > test5/lora_memory.csv &
NVIDIA_PID=$!

START=$(date +%s)
python3 /cache/home/kz498/Research/Train/lora/llama2/lora_llama2.py \
        --r 8 \
        --lora_alpha 16 \
        --lr 5e-3 \
        --num_train_epochs 4 \
        ...
        --warmup_ratio 0.03 \
        --lr_scheduler cosine \
        --output_dir ./test5/checkpoints \
        --save_dir ./test5/model \
...
awk -F',' 'NR>1 {print $2}' test5/lora_memory.csv | sort -n | tail -1
echo "================================"