#!/bin/bash
#SBATCH --job-name=lorta-finetune-test0
...
#SBATCH --output=test0/lorta_out.txt   # stdout — %j is the job ID
#SBATCH --error=test0/lorta_err.txt    # stderr separately so errors are easy to find
...
nvidia-smi --query-gpu=timestamp,memory.used,memory.free,utilization.gpu \
    --format=csv -l 5 > test0/lorta_memory.csv &
NVIDIA_PID=$!
...
awk -F',' 'NR>1 {print $2}' test0/lorta_memory.csv | sort -n | tail -1
echo "================================"