#!/bin/bash
#SBATCH --job-name=lorta-llama2-test7
...
#SBATCH --output=test7/out.txt   # stdout — %j is the job ID
#SBATCH --error=test7/err.txt    # stderr separately so errors are easy to find
...
mkdir -p ./test7/model ./test7/checkpoints

CHECKPOINT_DIR=./test0/checkpoints
RESUME_ARG=""
...
nvidia-smi --query-gpu=timestamp,memory.used,memory.free,utilization.gpu \
    --format=csv -l 5 > test7/memory.csv &
NVIDIA_PID=$!
...
python3 /cache/home/kz498/Research/Train/lorta/llama2/lorta_llama2.py \
        ...
        --warmup_ratio 0.03 \
        --lr_scheduler cosine \
        --output_dir ./test7/checkpoints \
        --save_dir ./test7/model \
...
awk -F',' 'NR>1 {print $2}' test7/memory.csv | sort -n | tail -1
echo "================================"