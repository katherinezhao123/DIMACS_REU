#!/bin/bash
#SBATCH --job-name=lorta-padded-llama2-test3
...
#SBATCH --output=test3/out.txt   # stdout — %j is the job ID
#SBATCH --error=test3/err.txt    # stderr separately so errors are easy to find
...
mkdir -p ./test3/model ./test3/checkpoints

# Monitor GPU memory
nvidia-smi --query-gpu=timestamp,memory.used,memory.free,utilization.gpu \
    --format=csv -l 5 > test3/memory.csv &
NVIDIA_PID=$!
...
python3 /cache/home/kz498/Research/Train/lorta_padded/llama2/lorta_padded_llama2.py \
        ...
        --warmup_ratio 0.06 \
        --lr_scheduler cosine \
        --output_dir ./test3/checkpoints \
        --save_dir ./test3/model \
...
awk -F',' 'NR>1 {print $2}' test3/memory.csv | sort -n | tail -1
echo "================================"