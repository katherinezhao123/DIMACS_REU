#!/bin/bash
#SBATCH --job-name=lorta-padded-llama2-test6
...
#SBATCH --output=test6/out.txt   # stdout — %j is the job ID
#SBATCH --error=test6/err.txt    # stderr separately so errors are easy to find
...
nvidia-smi --query-gpu=timestamp,memory.used,memory.free,utilization.gpu \
    --format=csv -l 5 > test6/memory.csv &
NVIDIA_PID=$!
...
awk -F',' 'NR>1 {print $2}' test6/memory.csv | sort -n | tail -1
echo "================================"