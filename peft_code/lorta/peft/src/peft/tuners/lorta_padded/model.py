...
from .config import LorTaPaddedConfig
from .gptq import dispatch_gptq
from .layer import Linear as LorTaPaddedLinear
from .layer import LorTaPaddedLayer, dispatch_default
...
class LorTaPaddedModel(BaseTuner):
    """
    Creates Low Rank Tensor Adapter (LorTa Padded) model from a pretrained transformers model.
    ...
    **Attributes**:
    ...
    def _compute_weights_from_tensor(self):
        ...
        for block_idx in range(self.model.config.num_hidden_layers):
            for proj, m_idx in [("q", 0), ("k", 1), ("v", 2), ("o", 3)]:
                n_heads = self.model.proj_n_heads[(block_idx, proj)]
                in_dim   = self.model.proj_in_dims[(block_idx, proj)]
                out_dim  = self.model.proj_out_dims[(block_idx, proj)]
                head_dim = out_dim // n_heads 

                A = self.model.lora_A * self.model.mask_A  # (max_in_dim, r)
                B = self.model.lora_B * self.model.mask_B 
                ...
                matrices = [
                    A
                    @ torch.diag(
                        self.model.lora_C_h[head_idx] * self.model.lora_C_m[m_idx] * self.model.lora_C_l[block_idx]
                    )
                    @ B 
                    for head_idx in range(n_heads)
                ]
                weights[self._map_layer_to_adapter(block_idx, proj)] = torch.cat(matrices, dim=1).T
    ...
    def inject_adapter(self, model: nn.Module, adapter_name: str):
        ...
        proj_to_nheads = {"q": num_heads, "k":num_kv_heads, "v": num_kv_heads, "o":num_heads}

        self.model.proj_in_dims = {}
        self.model.proj_out_dims = {}
        self.model.proj_n_heads = {}
        max_in_dim = 0
        max_head_dim = 0
        ...
        for layer_idx in range(num_layers):
            ...
            for proj, module in proj_modules.items():
                in_dim = module.weight.shape[1]
                out_dim = module.weight.shape[0]
                n_heads = proj_to_nheads[proj]
                head_dim = out_dim // n_heads

                self.model.proj_in_dims[(layer_idx, proj)] = in_dim
                self.model.proj_out_dims[(layer_idx, proj)] = out_dim
                self.model.proj_n_heads[(layer_idx, proj)] = n_heads
                
                max_in_dim = max(max_in_dim, in_dim)
                max_head_dim = max(max_head_dim, head_dim) 

        
        A_shape = (max_in_dim, peft_config.r)
        B_shape = (peft_config.r, max_head_dim)
        C_shape = (peft_config.r,)
        self.model.lora_C_l = nn.Parameter(torch.zeros((num_layers,) + C_shape))
        # nn.init.kaiming_uniform_(self.model.lora_C_l, a=math.sqrt(5) * peft_config.init_scale)
        nn.init.normal_(self.model.lora_C_l, mean=0.0, std=0.01)
        self.model.lora_C_h = nn.Parameter(torch.zeros((num_heads,) + C_shape))
        # nn.init.kaiming_uniform_(self.model.lora_C_h, a=math.sqrt(5) * peft_config.init_scale)
        nn.init.normal_(self.model.lora_C_h, mean=0.0, std=0.01)
        ...
        nn.init.normal_(self.model.lora_C_m, mean=0.0, std=0.01)
        ...
        #zeroes out rows and cols that are not the actual dimension
        mask_A = torch.zeros(A_shape)
        mask_B = torch.zeros(B_shape)

        for layer_idx in range(num_layers):
            for proj in ["q", "k", "v", "o"]:
                in_dim = self.model.proj_in_dims[(layer_idx, proj)]
                out_dim = self.model.proj_out_dims[(layer_idx, proj)]
                n_heads = self.model.proj_n_heads[(layer_idx, proj)]
                head_dim = out_dim // n_heads

                mask_A[:in_dim, :] = 1.0 
                mask_B[:, :head_dim] = 1.0 

        self.model.register_buffer("mask_A", mask_A)
        self.model.register_buffer("mask_B", mask_B)

        self._mark_only_adapters_as_trainable(model)
        
        def zero_pad_grads_A(grad):
            if grad is not None: 
                return grad*self.model.mask_A
            return grad

        def zero_pad_grads_B(grad):
            if grad is not None: 
                return grad *self.model.mask_B
            return grad 
        self.model.lora_A.register_hook(zero_pad_grads_A)
        self.model.lora_B.register_hook(zero_pad_grads_B)

    def weights_pre_forward_hook(self, target, args, kwargs, module_name):
        # pre-forward hook to inject weights
        # print(self.tensor_weights.keys())
        if not self.tensor_weights:  # recompute if not set
            self.tensor_weights = self._compute_weights_from_tensor()
        kwargs["adapter_weight"] = self.tensor_weights[module_name]
        return args, kwargs

    def _check_new_adapter_config(self, config: LorTaPaddedConfig) -> None:
        """
    ...
    def _prepare_model(self, peft_config: LorTaPaddedConfig, model: nn.Module):
        r"""
    ...
    def _create_and_replace(
        ...
        if isinstance(target, LorTaPaddedLayer) and not isinstance(target, AdaLoraLayer):
            target.update_layer(
                adapter_name,
                r,
                lora_alpha=alpha,
        ...
        for active_adapter in self.active_adapters:
            ...
            if bias == "all":
                ...
                for m in model.modules():
                    if isinstance(m, LorTaPaddedLayer) and hasattr(m, "bias") and m.bias is not None:
    ...
    def set_adapter(self, adapter_name: str | list[str]) -> None:
        ...
        """
        for module in self.model.modules():
            if isinstance(module, LorTaPaddedLayer):
                if module.merged:
    ...
    def _enable_peft_forward_hooks(self, *args, **kwargs):
        ...
        names_with_hooks = []
        for name, module in self.model.named_modules():
            if isinstance(module, LorTaPaddedLayer) or isinstance(module, LorTaPaddedLinear):
    ...
    def _unload_and_optionally_merge(
        ...
        modules_to_process = []
        for name, module in self.model.named_modules():
            if isinstance(module, (LorTaPaddedLayer, LorTaPaddedLinear)):
